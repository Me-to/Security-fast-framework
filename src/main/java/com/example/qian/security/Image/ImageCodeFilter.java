...
import com.example.qian.controller.ImageController;
...
public class ImageCodeFilter extends OncePerRequestFilter {
    ...
    private void validateCode(ServletWebRequest servletWebRequest) throws ServletRequestBindingException {
        ...
        if (StringUtils.isBlank(codeInRequest)) {
            throw new ImagaeCodeException("验证码不能为空！");
        }
        if (codeInSession == null) {
            throw new ImagaeCodeException("验证码不存在！");
        }
        if (codeInSession.isExpire()) {
            sessionStrategy.removeAttribute(servletWebRequest, ImageController.SESSION_KEY_IMAGE_CODE);
            throw new ImagaeCodeException("验证码已过期！");
        }
        if (!StringUtils.equalsIgnoreCase(codeInSession.getCode(), codeInRequest)) {
            throw new ImagaeCodeException("验证码不正确！");
        }
        sessionStrategy.removeAttribute(servletWebRequest, ImageController.SESSION_KEY_IMAGE_CODE);
